...
if(empty($errors))
{
	...
	$from_referido = $request->referido;
    $f_reg = date("Y-m-d");
    $h_reg = date("H:i");
   ...
   if($from_ciudad  == 'Pereira' || $from_ciudad  == 'Armenia' || $from_ciudad  == 'Manizales'){
	...
	if ( trim($from_name!='') || trim($from_correo!='') || trim($from_celular!='') ||  trim($from_tipo!='')){
		...
		VALUES (
		NULL ,  '$from_name',  '$comercial',  '$from_email',  '$from_celular',  '$from_cliente','$from_para',  '$from_tipo',  '$from_ciudad', '$from_referido','$h_reg', '$f_reg'
		);
...
if ( trim($from_name!='') || trim($from_email!='') || trim($from_celular!='') ||  trim($from_para!='')){
    ...
    $email_from = "news@econtainers.com.co";
		...
		$email_message .= "Referido: " . $from_referido . "\n\n";
